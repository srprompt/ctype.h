...
int main()
{
    ...
    while (str_exem[y])
    {
        d=str_exem[y]; //O caractere 'd' serve como auxiliar para fazer o teste dos caracteres recebidos da str_exem
        if(isspace(d)) //Verifica se o caractere 'd' é espaço
            d='\n'; //Faz 'd' receber uma quebra de linha caso seja um espaço
        putchar(d); //Coloca na tela o caractere 'd'
